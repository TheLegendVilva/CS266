...
void *scalarMultiplication(void* ptr){//scalar multiplication
    int c_;
...
void *vectorMultiplication(void* ptr){//multiplying input matrix with another matrix
    int c_;
...
void *transposeMatrix(void* ptr){//transpose of a matrix
    int i,j;
    int transpose[c][r];
    for (i = 0;i < r;i++){
        for (j = 0; j < c; j++){
